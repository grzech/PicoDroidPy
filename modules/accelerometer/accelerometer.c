...
// This is the function which will be called from Python as accelerometer.add_ints(a, b).
STATIC mp_obj_t accelerometer_add_ints(mp_obj_t a_obj, mp_obj_t b_obj) {
...
STATIC MP_DEFINE_CONST_FUN_OBJ_2(accelerometer_add_ints_obj, accelerometer_add_ints);

// This structure represents Timer instance objects.
typedef struct _accelerometer_Timer_obj_t {
    // All objects start with the base.
    mp_obj_base_t base;
    // Everything below can be thought of as instance attributes, but they
    // cannot be accessed by MicroPython code directly. In this accelerometer we
    // store the time at which the object was created.
    mp_uint_t start_time;
} accelerometer_Timer_obj_t;
...
STATIC mp_obj_t accelerometer_Timer_time(mp_obj_t self_in) {
    // The first argument is self. It is cast to the *accelerometer_Timer_obj_t
    // type so we can read its attributes.
    accelerometer_Timer_obj_t *self = MP_OBJ_TO_PTR(self_in);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_1(accelerometer_Timer_time_obj, accelerometer_Timer_time);

// This represents Timer.__new__ and Timer.__init__, which is called when
// the user instantiates a Timer object.
STATIC mp_obj_t accelerometer_Timer_make_new(const mp_obj_type_t *type, size_t n_args, size_t n_kw, const mp_obj_t *args) {
    // Allocates the new object and sets the type.
    accelerometer_Timer_obj_t *self = mp_obj_malloc(accelerometer_Timer_obj_t, type);
...
STATIC const mp_rom_map_elem_t accelerometer_Timer_locals_dict_table[] = {
    { MP_ROM_QSTR(MP_QSTR_time), MP_ROM_PTR(&accelerometer_Timer_time_obj) },
};
STATIC MP_DEFINE_CONST_DICT(accelerometer_Timer_locals_dict, accelerometer_Timer_locals_dict_table);

// This defines the type(Timer) object.
MP_DEFINE_CONST_OBJ_TYPE(
    accelerometer_type_Timer,
    MP_QSTR_Timer,
    MP_TYPE_FLAG_NONE,
    make_new, accelerometer_Timer_make_new,
    locals_dict, &accelerometer_Timer_locals_dict
    );
...
STATIC const mp_rom_map_elem_t accelerometer_module_globals_table[] = {
    ...
    { MP_ROM_QSTR(MP_QSTR_add_ints), MP_ROM_PTR(&accelerometer_add_ints_obj) },
    { MP_ROM_QSTR(MP_QSTR_Timer),    MP_ROM_PTR(&accelerometer_type_Timer) },
};
